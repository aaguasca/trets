...
__all__ = ["compute_weighted_mean", "normalised_excess_variance"]
...
def normalised_excess_variance(
    lc_table, alpha_level_sigmas, percent_sys_error=3, chi_squared_test=False
):
    """
    ...
    wmean_flux, wmean_flux_err, total_err = compute_weighted_mean(
        flux, flux_err, percent_sys_error=percent_sys_error
    )
    ...
    if chi_squared_test:
        ...
        chi2_value = np.sum(
            (time * (flux - wmean_flux) ** 2) / (total_err**2 * time.sum())
        )
